...
namespace ImGui
{
  ...
  {
    void draw_arc(
        ImVec2 center, float radius, float start_angle, float end_angle,
    ...
    struct knob
    {
      float radius;
      bool value_changed;
      bool value_deactivated_after_edit;
      ImVec2 center;
      bool is_active;
      bool is_hovered;
      float angle_min;
      ...
      knob(
          const char* _label, ImGuiDataType data_type, DataType* p_value,
          DataType v_min, DataType v_max, float speed, float _radius,
          const char* format, ImGuiKnobFlags flags, float _angle_min,
          float _angle_max)
      {
        radius = _radius;
        ...
        bool drag_vertical = !(flags & ImGuiKnobFlags_DragHorizontal)
                             && (flags & ImGuiKnobFlags_DragVertical
                                 || ImAbs(io.MouseDelta[ImGuiAxis_Y])
                                        > ImAbs(io.MouseDelta[ImGuiAxis_X]));
        ...
        value_changed = ImGui::DragBehavior(
            gid, data_type, p_value, speed, &v_min, &v_max, format,
            drag_behaviour_flags);

        // Notify ImGui that the value was changed so DeactivatedAfterEdit is triggered properly
        if (value_changed)
        {
          ImGui::MarkItemEdited(gid);
        }

        // Capture the deactivated state right here, before the Text Input overwrites it!
        value_deactivated_after_edit = ImGui::IsItemDeactivatedAfterEdit();
      ...
      void draw_dot(
          float size, float radius, float angle, color_set color, bool filled,
          int segments)
      {
      ...
      void draw_tick(
          float start, float end, float width, float angle, color_set color)
      {
      ...
      void draw_arc(
          float radius, float size, float start_angle, float end_angle,
          color_set color)
      {
        ...
        detail::draw_arc(
            center, track_radius, start_angle, end_angle, track_size,
            is_active ? color.active : (is_hovered ? color.hovered : color.base));
      }
    };
    ...
    knob<DataType> knob_with_drag(
        const char* label, ImGuiDataType data_type, DataType* p_value,
        DataType v_min, DataType v_max, float _speed, const char* format, float size,
        ImGuiKnobFlags flags, float angle_min, float angle_max)
    {
      if (flags & ImGuiKnobFlags_Logarithmic && v_min <= 0.0 && v_max >= 0.0)
      {
        ...
        v_max = ImMax(
            v_min,
            v_max); // this ensures that in the cornercase v_max is still at least ge v_min
        *p_value = ImMax(
            ImMin(*p_value, v_max),
            v_min); // this ensures that in the cornercase p_value is within the range
      }
      ...
      if (!(flags & ImGuiKnobFlags_NoTitle))
      {
        ...
        // Center title
        ImGui::SetCursorPosX(
            ImGui::GetCursorPosX() + (width - title_size[0]) * 0.5f);
      ...
      // Draw knob
      knob<DataType> k(
          label, data_type, p_value, v_min, v_max, speed, width * 0.5f, format,
          flags, angle_min, angle_max);

      // Track if either the knob OR the text input was deactivated
      bool is_deactivated_after_edit = k.value_deactivated_after_edit;
      ...
      if (!(flags & ImGuiKnobFlags_NoInput))
      {
        ...
        {
          is_deactivated_after_edit = true;
        }
      }

      ImGui::EndGroup();

      // Re-apply the aggregated deactivation state to the whole group so outer logic works!
      if (is_deactivated_after_edit)
      {
    ...
    color_set GetTrackColorSet()
    {
      ...
      return {
          colors[ImGuiCol_Button], colors[ImGuiCol_Button], colors[ImGuiCol_Button]};
    }
  } // namespace detail

  template<typename DataType>
  bool BaseKnob(
      const char* label, ImGuiDataType data_type, DataType* p_value, DataType v_min,
      DataType v_max, float speed, const char* format, ImGuiKnobVariant variant,
      float size, ImGuiKnobFlags flags, int steps, float angle_min, float angle_max)
  {
    auto knob = detail::knob_with_drag(
        label, data_type, p_value, v_min, v_max, speed, format, size, flags,
        angle_min, angle_max);
    ...
    case ImGuiKnobVariant_Wiper:
    {
      ...
      knob.draw_arc(
          0.8f, 0.41f, knob.angle_min, knob.angle_max, detail::GetTrackColorSet());

      if (knob.t > 0.01f)
      {
        knob.draw_arc(
            0.8f, 0.43f, knob.angle_min, knob.angle, detail::GetPrimaryColorSet());
      }
      break;
    }
    ...
    case ImGuiKnobVariant_WiperDot:
    {
      ...
      knob.draw_arc(
          0.85f, 0.41f, knob.angle_min, knob.angle_max, detail::GetTrackColorSet());
    ...
    {
      knob.draw_circle(
          0.3f - knob.t * 0.1f, detail::GetSecondaryColorSet(), true, 16);

      if (knob.t > 0.01f)
      {
        knob.draw_arc(
            0.4f, 0.15f, knob.angle_min - 1.0f, knob.angle - 1.0f,
            detail::GetPrimaryColorSet());
        knob.draw_arc(
            0.6f, 0.15f, knob.angle_min + 1.0f, knob.angle + 1.0f,
            detail::GetPrimaryColorSet());
        knob.draw_arc(
            0.8f, 0.15f, knob.angle_min + 3.0f, knob.angle + 3.0f,
            detail::GetPrimaryColorSet());
      }
      break;
    }
  ...
  bool Knob(
      const char* label, float* p_value, float v_min, float v_max, float speed,
      const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags,
      int steps, float angle_min, float angle_max)
  {
    return BaseKnob(
        label, ImGuiDataType_Float, p_value, v_min, v_max, speed, format, variant,
        size, flags, steps, angle_min, angle_max);
  }

  bool KnobInt(
      const char* label, int* p_value, int v_min, int v_max, float speed,
      const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags,
      int steps, float angle_min, float angle_max)
  {
    return BaseKnob(
        label, ImGuiDataType_S32, p_value, v_min, v_max, speed, format, variant,
        size, flags, steps, angle_min, angle_max);
  }
} // namespace ImGui
